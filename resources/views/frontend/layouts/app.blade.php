...
$.ajax({
    ...
    success: function (data) {
        ...
        let base_price = parseFloat(
            data.price.toString().replace(/,/g, '').replace(/[^0-9.]/g, '')
        ) || 0;
        ...
        // FABRIC hidden selected option
        $('input[type="hidden"][name^="addons"]').each(function(){

            let optionId = $(this).val();

            if(optionId){

                let fabricBtn = $('.fabric-color-box[data-optionid="'+optionId+'"]');

                if(fabricBtn.length){

                    let fabricPrice = parseFloat(fabricBtn.data('price')) || 0;
        ...
        // MULTIPLY BY QTY
        addon_total = addon_total * qty;

        // FINAL PRICE
        let final_price = base_price + addon_total;

        // UPDATE PRICE UI
        $('.js-product-total-price').html(
        ...
        $('#option-choice-form #chosen_price').html(
            '£ ' + final_price.toFixed(2)
        );
...
//                         $('.js-product-total-price').html('£ ' + final_price.toFixed(2));

//                         // OPTIONAL chosen price update
//                         $('#option-choice-form #chosen_price_div').removeClass('d-none');

//                         $('#option-choice-form #chosen_price').html('£ ' + final_price.toFixed(2));
