					...
					<tr>
						<td class="text-left" style="width:50%;"> 
							<table>
								...
								<tr><td class="gry-color small">{{ $shipping_address->address }},<br> {{ $shipping_address->city }} - {{ $shipping_address->postal_code }} @if(isset(json_decode($order->shipping_address)->state)) {{ json_decode($order->shipping_address)->state }} @endif<br>  {{ $shipping_address->country }}</td></tr>
						...
						<td class="text-left" style="width:50%;">
							<table >
								<tr><td class="strong small gry-color">{{ translate('Ship By') }}:</td></tr>
								<tr><td class="strong">{{ $order->shop->name }}</td></tr>
								<tr><td class="gry-color small">{{ $order->shop->address }},<br> {{ $order->shop->city->name }} - {{ $order->shop->postal_code }} {{ $order->shop->state->name }},<br>  {{ $order->shop->country->name }}</td></tr>
								<tr><td class="gry-color small">{{ translate('Email') }}: {{ $order->shop->email }}</td></tr>
							</table>
						</td>
					</tr>
				</table>
	    ...
	    <div style="padding:0 1.5rem;">
	        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        ...
			        <tr>
			            <th class="text-left strong">{{ translate('Grand Total') }} <small> (20% VAT INCURRED)</small></th>
