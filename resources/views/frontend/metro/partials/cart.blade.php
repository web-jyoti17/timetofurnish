...
<!-- Cart Button -->
<a href="javascript:void(0)" class="d-flex align-items-center text-dark px-3 h-100 position-relative" data-toggle="dropdown" data-display="static" title="{{translate('Cart')}}">
    <span class="position-relative">
        <svg class="icon-bag" aria-hidden="true" focusable="false" role="presentation" xmlns="http://www.w3.org/2000/svg" width="22" height="24" viewBox="0 0 22 24" fill="none">
            <path d="M6.91699 10.993V4.95104C6.91699 2.72645 8.70785 0.923065 10.917 0.923065C13.1261 0.923065 14.917 2.72645 14.917 4.95104V10.993" stroke="#1a1a1a" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
            <path fill-rule="evenodd" clip-rule="evenodd" d="M18.131 23.0769C19.6697 23.0769 20.917 21.8209 20.917 20.2714C20.9174 20.1285 20.9067 19.9857 20.885 19.8445L19.221 8.95686C19.0463 7.81137 18.0679 6.96551 16.917 6.96503H4.917C3.76678 6.96536 2.78859 7.81016 2.613 8.95485L0.949001 19.8545C0.927336 19.9958 0.916636 20.1386 0.917001 20.2815C0.92251 21.827 2.16823 23.0769 3.703 23.0769H18.131Z" stroke="#1a1a1a" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
        </svg>
        @if($cart_count > 0)
            <span class="badge badge-primary badge-circle position-absolute" style="top:-6px; right:-8px; font-size:10px; min-width:18px; height:18px; line-height:18px;">{{ $cart_count }}</span>
        @endif
    </span>
</a>

<!-- Cart Dropdown - Redesigned (Simple, modern, minimal, responsive, no heavy shadows, subtle radius) -->
<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg p-0 stop-propagation cart-dropdown-minimal" style="min-width: 360px; max-width: 400px;">
    @if ($cart_count > 0)
        <!-- Header -->
                    ...
                    <li class="list-group-item px-4 py-3 border-0 cart-item-minimal">
                        <div class="d-flex flex-column">
                            <div class="d-flex align-items-center">
                                ...
                                <a href="{{ route('product', $product->slug) }}" class="flex-shrink-0 mr-3 cart-thumb-minimal">

                                <img src="{{ static_asset('assets/img/placeholder.jpg') }}"
                                    data-src="{{ uploaded_asset($product->thumbnail_img) }}"
                                    class="img-fit lazyload simple-cart-img"
                                    width="64" height="64"
                                    alt="{{ $product->getTranslation('name') }}"
                                    onerror="this.onerror=null;this.src='{{ static_asset('assets/img/placeholder.jpg') }}';">
                                    <a href="{{ route('product', $product->slug) }}" class="text-dark text-decoration-none w-100">
                                        ...
                                        <div class="fs-13 text-muted">
                                            {{ $cartItem['quantity'] }} ×
                                            <span class="fw-600 text-primary">£{{ number_format($product_price, 2) }}</span>
                                        </div>
                            ...
                            <div class="flex-grow-1 min-w-0">
                                ...
                                @if($has_addons && is_array($addons) && count($addons) > 0)
                                    <button class="btn btn-sm btn-link p-0 mt-2 fs-12 fw-500 text-primary text-decoration-none minimal-addon-toggle"
                                            type="button"
                                            data-toggle="collapse"
                                            data-target="#addonCollapse{{ $cartItem['id'] }}"
                                            aria-expanded="false"
                                            aria-controls="addonCollapse{{ $cartItem['id'] }}">
                                        <i class="la la-plus-circle mr-1"></i> {{ translate('View Addons') }} ({{ count($addons) }})
                                    ...
                                    <div class="collapse mt-2" id="addonCollapse{{ $cartItem['id'] }}">
                                        <div class="p-2 addoncolor simple-addon-list">
                                            @foreach($addons as $addon)
                                                ...
                                                @if(is_array($addon))
                                                    <div class="d-flex justify-content-between align-items-start fs-12 text-black mb-1 addon-row">
                                                        <span class="addon-name-text">
                                                            <i class="la la-check-circle fs-12 mr-1 text-success"></i>
                                                          <strong> {{ $addon['addon_name'] }}:</strong> {{ $addon['name']?? $addon[0]?? '' }}
                                                        </span>
                                                        <span class="fw-500 text-dark addon-price-text flex-shrink-0 ml-2">
        ...
        <!-- Subtotal -->
        <div class="px-2 py-2 border-top cart-totals-minimal" style="background: #dfd1be; border-radius: 0; margin-bottom: 0;">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <span class="fs-15 fw-600 text-muted" style="letter-spacing: 0.01em;">{{ translate('Subtotal') }}</span>
                <span class="fs-22 fw-800 text-dark">£{{ number_format($total, 2) }}</span>
            </div>
            <div class="d-flex gap-2 flex-row align-items-stretch" style="gap: 8px;">
                <a href="{{ route('cart') }}"
                   class="cart-btn-outline d-flex align-items-center flex-fill justify-content-center">
...
>
        ...
        <style>
            ...
            .cart-action-btn:last-child, .cart-action-btn:not(:first-child) {
                background: #22222c;
                color: #fff;
                border: none;
            }
    ...
    @else
        ...
        <div class="text-center py-5 px-4">
            ...
            <p class="fs-13 text-muted mb-3">{{ translate('Looks like you haven’t added anything to your cart yet.') }}</p>
            <a href="{{ route('home') }}" class="btn btn-primary btn-sm rounded-pill px-4">{{ translate('Start Shopping') }}</a>
        </div>
    @endif
</div>
...
.remove-btn-minimal:hover, .remove-btn-minimal:focus {
...
.cart-dropdown-no-scroll, .cart-products-list {
...
.product-name-text, .addon-name-text {
...
@media (max-width: 500px) {
    ...
    .cart-header, .simple-subtotal-row {
