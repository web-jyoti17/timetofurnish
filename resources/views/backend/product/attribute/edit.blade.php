            ...
            <div class="card-body p-0">
                ...
                <form class="p-4" action="{{ route('attributes.update', $attribute->id) }}" method="POST">
                    ...
                    <div class="form-group mb-3">
                        <label for=""><b>{{ translate('Categories') }}</b></label>
                        @foreach($categories as $val_cat)  <br>
                            <input type="checkbox" id="{{ $val_cat->name }}" value="{{ $val_cat->id }}" name="categories[]" @if(in_array($val_cat->id, $AttributeCategory)) checked @endif />
                            <label for="{{ $val_cat->name }}"> {{ $val_cat->name }}</label>
                        @endforeach
                    </div>
