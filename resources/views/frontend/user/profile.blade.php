    ...
    <div class="card rounded-0 shadow-none border">
        ...
        <div class="card-body">
            <form action="{{ route('user.profile.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <!-- Phone-->
               <div class="form-group row">
                         <label class="col-md-2 col-form-label fs-14"> {{ translate('Mobile Number') }}  </label>
                    <div class="col-md-10">
                       <input type="text"   class="form-control rounded-0"  placeholder="{{ translate('Enter Your Mobile Number')}}"  name="phone"  maxlength="14"  inputmode="numeric"  oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0,14)"
                ...
                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn  borderbtn rounded-0 w-150px mt-3">{{translate('Update Profile')}}</button>
                </div>
            </form>
        </div>
    </div>
            ...
            @foreach (Auth::user()->addresses as $key => $address)
                <div class="">
                    <div class="border p-4 mb-4 position-relative">
                        ...
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-6 text-secondary">{{ translate('Flat/building number or name') }}:</span>
                            <span class="col-md-6 text-dark">{{ $address->flat }}</span>
                        </div>
                        
                        
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-6 text-secondary">{{ translate('Street') }}:</span>
                            <span class="col-md-6 text-dark">{{ $address->street }}</span>
                        </div>
                        
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-6 text-secondary">{{ translate('Town or City') }}:</span>
                            <span class="col-md-6 text-dark">{{ $address->city_id }}</span>
                        </div>
                        
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-6 text-secondary">{{ translate('Post Code') }}:</span>
                            <span class="col-md-6 text-dark">{{ $address->postal_code }}</span>
                        </div>
                        ...
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-6 text-secondary">{{ translate('Mobile Number') }}:</span>
                              <span class="col-md-6 text-dark">    {{ $address->phone }}  </span>
                        </div>
                        
                        <div class="row fs-14 mb-2 mb-md-0">
                            <span class="col-md-6 text-secondary">{{ translate('Landline Number') }}:</span>
                              <span class="col-md-6 text-dark">    {{ $address->landline_no}}  </span>
                        </div>
          ...
          <div class="card-body">
              <div class="row">
                  ...
                  <div class="col-md-10">
                      ...
                      <div class="form-group mb-0 text-right">
                          <button type="submit" class="btn borderbtn rounded-0 w-150px mt-3">{{translate('Update Email')}}</button>
                      </div>
                  </div>
              </div>
          </div>
...
<style type="text/css">
    .text-secondary{line-height:27px; letter-spacing:1;}
</style>
@endsection
