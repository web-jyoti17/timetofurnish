...
class ProductService
{
    public function store(array $data)
    {
        ...
        if (!empty($collection['date_range'])) {
    ...
    public function update(array $data, Product $product)
    {
        ...
        if (!empty($collection['date_range']) && str_contains($collection['date_range'], ' to ')) {
            $date_var = explode(" to ", $collection['date_range']);
            $discount_start_date = strtotime($date_var[0]);
            $discount_end_date   = strtotime($date_var[1]);
        }
        ...
        if ($collection['meta_description'] == null) {
            ...
            if (!empty($collection['lang']) && $collection['lang'] != env("DEFAULT_LANGUAGE")) {
