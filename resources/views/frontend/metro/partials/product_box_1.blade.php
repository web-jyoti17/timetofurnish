...
@endphp
<div class="aiz-card-box h-auto bg-white p-3 hov-scale-img modern-product-card position-relative 3">
    <a href="{{ $product_url }}" class="position-absolute w-100 h-100"
        style="top:0;left:0;z-index:1;" aria-label="{{ $product->getTranslation('name') }}"></a>
        ...
        <div class="position-relative h-140px h-md-160px img-fit overflow-hidden modern-product-img-wrap">
            ...
            @endphp
            <!-- Image -->
            <a href="{{ $product_url }}" class="d-block h-100 position-relative" style="z-index:2;">
        ...
        <div class="p-2 p-md-3 text-left">
            ...
            <h3 class="fw-500 fs-14 text-truncate-2 lh-1-4 mb-1 modern-product-title">
                <a href="{{ $product_url }}" class="d-block text-reset hov-text-primary position-relative"
                    style="letter-spacing:0.3px !important;z-index:2;"
                    ...
                    @if ($product->auction_product == 0)
                        @if ($active_offer)
                            ...
                            <div class="d-flex align-items-center flex-wrap" style="gap: 2px;">
                                ...
                                @endif
                                <span
                                    class="fw-700 fs-14 text-primary">{{ home_discounted_base_price($product) }}</span>
                            </div>
                        @else
                            <!-- Previous price -->
                            ...
                            <div class="price d-block">
                                <span
                                    class="fw-700 text-primary fs-14">{{ home_discounted_base_price($product) }}</span>
                            </div>
                        @endif
                    @endif
                    @if ($product->auction_product == 1)
