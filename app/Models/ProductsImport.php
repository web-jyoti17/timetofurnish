...
class ProductsImport implements ToCollection, WithHeadingRow, WithValidation, ToModel
{
    ...
    public function collection(Collection $rows)
    {
        ...
        if ($canImport) {
            foreach ($rows as $row) {
                ...
                $mainCategoryProduct = $this->createProduct($row, $user, $approved, $row['category_id']);
                ...
                if (!empty($row['sub_category_ids'])) {
                    $subCategoryIds = explode(',', $row['sub_category_ids']);
                    array_push($subCategoryIds, $row['category_id']);
                    $mainCategoryProduct->categories()->attach($subCategoryIds);
                    // foreach ($subCategoryIds as $subCategoryId) {
                    //     $this->createProduct($row, $user, $approved, $subCategoryId, $mainCategoryProduct->id);
                    // }
                }
    ...
    protected function createProduct($row, $user, $approved, $categoryId, $parentId = null)
    {
         $productId = Product::create([
            'name' => $row['name'],
            'description' => $row['description'],
    ...
    {
        try {
            if (Str::contains($url, 'madefurniture.com')) {
                $afterPublic = Str::after($url, 'public/');
                $image = Upload::where('file_name', $afterPublic)->first();
                if($image){
                    return $image->id;
                }else{
                    $upload = new Upload;
