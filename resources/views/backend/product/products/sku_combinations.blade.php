            ...
            @foreach ($combinations as $key => $combination)
                ...
                @if(strlen($str) > 0)
                   <tr class="variant" data-variant-values='@json($combination)'>
                        ...
                        <td style="border: 1px solid #eae9e9;">
                            <input type="number" lang="en" name="qty_{{ $str }}" value="{{ $old_values->{'qty_'.$str} ?? 1 }}" min="1" step="1" class="form-control var_qty rounded-pill" oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,4); if (parseInt(this.value || 0, 10) <= 0) this.value = '';" required>
                        </td>
                        <td style="border: 1px solid #eae9e9;">
